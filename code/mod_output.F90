...
module mod_output
  ...
  use mod_slabocean
  use mod_cloud_s1
  use mod_che_indices   !!! xiaoxie 2017
  
  implicit none

  private

  public :: output,output_new
...
!!!!! yibs outputs     
        if ( associated(srf_co2flux_out) ) &
          srf_co2flux_out = srf_co2flux_out*rnsrf_for_srffrq
        if ( associated(srf_npp_out) ) &
          srf_npp_out = srf_npp_out*rnsrf_for_srffrq
        if ( associated(srf_gpp_out) ) &
          srf_gpp_out = srf_gpp_out*rnsrf_for_srffrq
        if ( associated(srf_swd_out) ) &
          srf_swd_out = srf_swd_out*rnsrf_for_srffrq
        if ( associated(srf_swf_out) ) &
          srf_swf_out = srf_swf_out*rnsrf_for_srffrq
        if ( associated(srf_Rauto_out) ) &
          srf_Rauto_out = srf_Rauto_out*rnsrf_for_srffrq
        !if ( associated(srf_soilresp_out) ) &
        !  srf_soilresp_out = srf_soilresp_out*rnsrf_for_srffrq
        if ( associated(srf_lai_out) ) &
          srf_lai_out = srf_lai_out*rnsrf_for_srffrq
!!!!! yibs debug values 
!!!!! below is disabled srf2d 35-48        
        if ( associated(srf_TairC_out) ) &
          srf_TairC_out = srf_TairC_out*rnsrf_for_srffrq
        if ( associated(srf_TcanopyC_out) ) &
          srf_TcanopyC_out = srf_TcanopyC_out*rnsrf_for_srffrq
        if ( associated(srf_Qf_out) ) &
          srf_Qf_out = srf_Qf_out*rnsrf_for_srffrq
        if ( associated(srf_P_mbar_out) ) &
          srf_P_mbar_out = srf_P_mbar_out*rnsrf_for_srffrq
        if ( associated(srf_Ca_out) ) &
          srf_Ca_out = srf_Ca_out*rnsrf_for_srffrq
        if ( associated(srf_yCh_out) ) &
          srf_yCh_out = srf_yCh_out*rnsrf_for_srffrq
        if ( associated(srf_U_out) ) &
          srf_U_out = srf_U_out*rnsrf_for_srffrq
        if ( associated(srf_IPARdir_out) ) &
          srf_IPARdir_out = srf_IPARdir_out*rnsrf_for_srffrq
        if ( associated(srf_IPARdif_out) ) &
          srf_IPARdif_out = srf_IPARdif_out*rnsrf_for_srffrq
        if ( associated(srf_o3c_out) ) &
          srf_o3c_out = srf_o3c_out*rnsrf_for_srffrq
        if ( associated(srf_elev_out) ) &
          srf_elev_out = srf_elev_out*rnsrf_for_srffrq
        if ( associated(srf_inlat_out) ) &
          srf_inlat_out = srf_inlat_out*rnsrf_for_srffrq
        if ( associated(srf_plant_out) ) &
          srf_plant_out = srf_plant_out*rnsrf_for_srffrq
        if ( associated(srf_harvest_out) ) &
          srf_harvest_out = srf_harvest_out*rnsrf_for_srffrq        
!!!!! below is disabled srf3d 18-21 
        if ( associated(srf_soilt_out) ) then
              srf_soilt_out =srf_soilt_out*rnsrf_for_srffrq
        end if
        if ( associated(srf_soilm_out) ) then
              srf_soilm_out = srf_soilm_out*rnsrf_for_srffrq
        end if
        if ( associated(srf_soilp_out) ) then
              srf_soilp_out =srf_soilp_out *rnsrf_for_srffrq
        end if
        if ( associated(srf_soili_out) ) then
              srf_soili_out =srf_soili_out*rnsrf_for_srffrq
        end if
!!!!! below is enabled srf3d 22-26
        if ( associated(srf_inveg_out) ) then  !!!! xiaoxie added debug yibs input
              srf_inveg_out = srf_inveg_out*rnsrf_for_srffrq
        end if
        if ( associated(srf_inpop_out) ) then
              srf_inpop_out =srf_inpop_out*rnsrf_for_srffrq
        end if
        if ( associated(srf_inh_out) ) then
              srf_inh_out = &
              srf_inh_out*rnsrf_for_srffrq
        end if
        if ( associated(srf_indbh_out) ) then
              srf_indbh_out = &
              srf_indbh_out*rnsrf_for_srffrq
        end if
        if ( associated(srf_incrad_out) ) then
              srf_incrad_out = &
              srf_incrad_out*rnsrf_for_srffrq
        end if
!!!!! below is disabled srf3d 27-31
        if ( associated(srf_intexture_out) ) then
              srf_intexture_out  = &
              srf_intexture_out*rnsrf_for_srffrq
        end if
        if ( associated(srf_inlai_out) ) then
              srf_inlai_out = &
              srf_inlai_out*rnsrf_for_srffrq
        end if
        if ( associated(srf_inalbedo_out) ) then
              srf_inalbedo_out = &
              srf_inalbedo_out*rnsrf_for_srffrq
        end if
        if ( associated(srf_incpool_out) ) then
              srf_incpool_out  = &
              srf_incpool_out*rnsrf_for_srffrq
        end if
        if ( associated(srf_intpool_out) ) then
              srf_intpool_out = &
              srf_intpool_out*rnsrf_for_srffrq
        end if       
...
!!!!! varibles listed below are averaged values !!!!!
!!!!! yibs outputs     
        if ( associated(srf_co2flux_out) ) srf_co2flux_out = d_zero
        if ( associated(srf_npp_out) ) srf_npp_out = d_zero
        if ( associated(srf_gpp_out) ) srf_gpp_out = d_zero
        if ( associated(srf_swd_out) ) srf_swd_out = d_zero
        if ( associated(srf_swf_out) ) srf_swf_out = d_zero
        if ( associated(srf_Rauto_out) ) srf_Rauto_out = d_zero
        !if ( associated(srf_soilresp_out) ) srf_soilresp_out = d_zero
        if ( associated(srf_lai_out) ) srf_lai_out = d_zero
!!!!! yibs debug values  
!!!!! below is disabled srf2d 35-48        
        if ( associated(srf_TairC_out) ) srf_TairC_out = d_zero
        if ( associated(srf_TcanopyC_out) ) srf_TcanopyC_out = d_zero
        if ( associated(srf_Qf_out) ) srf_Qf_out = d_zero
        if ( associated(srf_P_mbar_out) ) srf_P_mbar_out = d_zero
        if ( associated(srf_Ca_out) ) srf_Ca_out = d_zero
        if ( associated(srf_yCh_out) ) srf_yCh_out = d_zero
        if ( associated(srf_U_out) ) srf_U_out = d_zero
        if ( associated(srf_IPARdir_out) ) srf_IPARdir_out = d_zero
        if ( associated(srf_IPARdif_out) ) srf_IPARdif_out = d_zero
        if ( associated(srf_o3c_out) ) srf_o3c_out = d_zero
        if ( associated(srf_elev_out) ) srf_elev_out = d_zero
        if ( associated(srf_inlat_out) ) srf_inlat_out = d_zero
        if ( associated(srf_plant_out) ) srf_plant_out = d_zero
        if ( associated(srf_harvest_out) ) srf_harvest_out = d_zero       
!!!!! below is disabled srf3d 18-21 
        if ( associated(srf_soilt_out) )srf_soilt_out = d_zero
        if ( associated(srf_soilm_out) )srf_soilm_out = d_zero
        if ( associated(srf_soilp_out) )srf_soilp_out = d_zero
        if ( associated(srf_soili_out) )srf_soili_out = d_zero
!!!!! below is enabled srf3d 22-26         
        if ( associated(srf_inveg_out) )srf_inveg_out = d_zero
        if ( associated(srf_inpop_out) )srf_inpop_out = d_zero
        if ( associated(srf_inh_out) )  srf_inh_out = d_zero
        if ( associated(srf_indbh_out) )srf_indbh_out = d_zero
        if ( associated(srf_incrad_out) )srf_incrad_out= d_zero
!!!!! below is disabled srf3d 27-31
        if ( associated(srf_intexture_out)) srf_intexture_out = d_zero
        if ( associated(srf_inlai_out) )srf_inlai_out = d_zero
        if ( associated(srf_inalbedo_out) ) srf_inalbedo_out= d_zero
        if ( associated(srf_incpool_out) ) srf_incpool_out  = d_zero
        if ( associated(srf_intpool_out) ) srf_intpool_out = d_zero  
!!!!!  xiaoxie add for yibs end !!!!!          
      end if
    end if
    ...
    if ( che_stream > 0 ) then
      if ( ldoche ) then
        ...
        do itr = 1 , ntr
          ...
          !if(itr.eq.ico2 &  !!! xiaoxie 2017
          ! .or.itr.eq.ich4 &  !!! output species
          ! .or.itr.eq.io3  &
          ! .or.itr.eq.ico  &
          ! .or.itr.eq.iso2 &
          ! .or.itr.eq.ino2 &
...
subroutine output_new
    implicit none
    logical :: ldoatm , ldosrfyibs , ldorad , ldoche
    logical :: ldosav , ldolak , ldosub
    logical :: ldoslab
    logical :: lstartup
    integer(ik4) :: i , j , k , itr
    real(rkx) :: rnsrf_for_srffrq2
#ifdef DEBUG
    character(len=dbgslen) :: subroutine_name = 'output'
    integer(ik4) , save :: idindx = 0
    call time_begin(subroutine_name,idindx)
#endif
    rnsrf_for_srffrq2 = 1/144.
    if ( srfyibs_stream > 0 ) then
        ps_out = d_1000*(sfs%psa(jci1:jci2,ici1:ici2)+ptop)
        ! Averaged values
!!!!! xiaoxie add for yibs !!! 
!!!!! varibles listed below are averaged values !!!!!
!!!!! yibs outputs     
        if ( associated(srfyibs_co2flux_out) ) &
          srfyibs_co2flux_out = srfyibs_co2flux_out*rnsrf_for_srffrq2
        if ( associated(srfyibs_npp_out) ) &
          srfyibs_npp_out = srfyibs_npp_out*rnsrf_for_srffrq2
        if ( associated(srfyibs_gpp_out) ) &
          srfyibs_gpp_out = srfyibs_gpp_out*rnsrf_for_srffrq2
        if ( associated(srfyibs_Rauto_out) ) &
          srfyibs_Rauto_out = srfyibs_Rauto_out*rnsrf_for_srffrq2
        if ( associated(srfyibs_lai_out) ) &
          srfyibs_lai_out = srfyibs_lai_out*rnsrf_for_srffrq2

        call write_record_output_stream(srfyibs_stream,idatex)
        if ( myid == italk ) &
          write(stdout,*) 'srfyibs variables written at ' , tochar(idatex)


!!!!! xiaoxie add for yibs !!! 
!!!!! varibles listed below are averaged values !!!!!
!!!!! yibs outputs     
        if ( associated(srfyibs_co2flux_out) ) srfyibs_co2flux_out = d_zero
        if ( associated(srfyibs_npp_out) ) srfyibs_npp_out = d_zero
        if ( associated(srfyibs_gpp_out) ) srfyibs_gpp_out = d_zero
        if ( associated(srfyibs_Rauto_out) ) srfyibs_Rauto_out = d_zero
        !if ( associated(srfyibs_soilresp_out) ) srfyibs_soilresp_out = d_zero
        if ( associated(srfyibs_lai_out) ) srfyibs_lai_out = d_zero
!!!!! yibs debug values           
    end if
