#define GCM_DRV
      ...
      module yibs_prescribed_drv_geo
!@sum yibs_prescr_veg_geo   Module of routines to read geographic cover, max LAI, canopy heights, and calculate plant densities and carbon pools.  Also, routine to read geographic initial LAI.
!@auth N.Y.Kiang
      ...
      public init_yibsvegdata_geo
      public init_yibs_laimax_geo
...
!      end function IJzero
      ...
      subroutine init_yibs_laimax_geo(IM,JM,I0,I1,J0,J1,laimax)
...
!@auth N.Y.Kiang
      ...
      end subroutine init_yibs_laimax_geo


!-----------------------------------------------------------------

      subroutine init_yibsvegdata_geo( IM,JM,I0,I1,J0,J1
...
!@sum get_yibsvegdata_geo Called by main driver to initialize
...
!      call get_yibs_laimax_geo(IM,JM,I0,I1,J0,J1,laimaxgeo) !Read file
...
!               cpooldata(p,FR,i,j) = cpooldata(p,FOL,i,j)
 ...
 !!     L3(:,:) = 0.d0
 !!     L4(:,:) = 0.d0
 !!     open(9950,file='YIBS16dens.ij',form='unformatted',status='unknown')
 !!     open(9951,file='YIBS16cpools.ij',
 !!    &     form='unformatted',status='unknown')
 !!     do p=1,N_COVERTYPES
 !!           TITLE = trim(YIBS_title(p)(1:50))//' popdata (#/m2)'
 !!           L2 = popdata(p,:,:)
 !!           write(9951) TITLE, L2
 !!           L3(:,:) = 0.0
 !!        do k = 1,N_BPOOLS
 ...
 !!           TITLE = trim(YIBS_title(p)(1:50))//'  '//YIBS_cpool_title(k)//
 !!    &           '   '//'Biomass (kgC/m2)'
 !!           write(9950) TITLE, L2(:,:)
 !!           TITLE = trim(YIBS_title(p)(1:50))//'  '//
 !!    &           YIBS_cpool_title(k)//'  (kgC/plant)'
 !!           L4(:,:) =cpooldata(p,k,:,:)*1.d-3
 !!           write(9951) TITLE, L4
 !!           L3(:,:) = IJADD4(I0,I1,J0,J1,L3(:,:),L2(:,:))
 !!           L1(:,:) = IJADD4(I0,I1,J0,J1,L1(:,:),L2(:,:))
 !!        enddo
 !!        TITLE = trim(YIBS_title(p)(1:50))//'  Biomass  '//'(kgC/m2)'
 !!        write(9950) TITLE, L3
 !!     enddo
 ...
 !!     close(9951)
      ...
      end subroutine init_yibsvegdata_geo

      end module yibs_prescribed_drv_geo
