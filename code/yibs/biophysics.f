...
!@+   up leaf-level fluxes to the canopy.
      ...
      use FarquharBBpspar
      use yibs_debug_mod, only : yibs_d
...
!SOILMOIST_OLD
     ...
     &          cop%fracroot, pp%cellptr%fice(:), cop%stressH2Ol(:))
         ...
         ! print*,'dtsec,cop%pft,psdrvpar%Pa,psdrvpar%Tc
      !i         ,O2frac*psdrvpar%Pa
      !i         ,cop%stressH2O,cop%Sacclim,cop%llspan:',
      !&         dtsec,cop%pft,psdrvpar%Pa,psdrvpar%Tc
      !i         ,O2frac*psdrvpar%Pa
      !i         ,cop%stressH2O,cop%Sacclim,cop%llspan
         ! print *,"before calc_pspar, pspar:",pspar
          call calc_Pspar(dtsec,cop%pft,psdrvpar%Pa,psdrvpar%Tc
     i         ,O2frac*psdrvpar%Pa
     i         ,cop%stressH2O,cop%Sacclim,cop%llspan)
         ! print *,"after calc_pspar, pspar:",pspar
...
!        call Allocate_NPP_to_labile(dtsec, cop)
        ...
        yibs_d%vf(cop%pft) = pp%area/(3600*24*1000.d0)
        yibs_d%Anet(cop%pft) = Anet
        yibs_d%Atot(cop%pft) = Atot
        yibs_d%Rd(cop%pft) = Rd
        yibs_d%GCANOPY(cop%pft) = GCANOPY
        yibs_d%TRANS_SW(cop%pft) = TRANS_SW/(3600*24*1000.d0)
        yibs_d%LAI(cop%pft) = cop%LAI/(3600*24*1000.d0)
        yibs_d%GPP(cop%pft) = cop%GPP
        yibs_d%R_auto(cop%pft) = cop%R_auto
