...
!      real*8 :: Je1, Jc1, Js1   !Assimilation of CO2, 3 limiting cases
      ...
      !if ( IPAR > 2.50 ) then
      !print*,"count=",counter,pft,IPAR,ca,Tl,Pa,rh,gb
      !endif
      ...
      if ( IPAR < .000001d0 ) then
        ...
        Atot0= 0.d0
        Anet = - Rd
        cs = ca - Anet*1.37d0/gb  !!! xiaoxie
        !if(cs.lt.0) print*,'here cs<0, for first',ca,Anet,gb
...
!      call Ci_Jc(ca,gb,rh,IPAR,Pa,pspar, Rd,O2pres, cic, Jc1)
      ...
      if ( pspar%first_call ) then
         if (pfpar(pspar%pft)%pst.eq.C3) then
            ...
            !print*,'before ci_cubic no.1'
            call ci_cubic(ca,rh,gb,Pa,Rd,a1c,f1c,pspar,Ac)
            !print*,'after ci_cubic no.1'
            !if ( Ac >= -Rd ) write(578,*) Axxx, Ac, Ac - Axxx
           ! write(*,*) "Ac", ca,rh,gb,Pa,Rd,pspar,Ac
...
!      call Ci_Je(ca,gb,rh,IPAR,Pa, pspar, Rd, cie, Je1)
     ...
     &        need_isoprene ) then
            ...
            !print*,'before ci_cubic no.2'
            call ci_cubic(ca,rh,gb,Pa,Rd,a1e,f1e,pspar,Ae)
            !print*,'after ci_cubic no.2'
            !write(*,*) "Ae", ca,rh,gb,Pa,Rd,a1e,f1e,pspar,Ae 
...
#if (defined O3DEP_UPTAKE) || (defined O3DEP_UPTAKE_OFFLINE)
      cs = ca - Anet*1.37d0/gb !!! xiaoxie 
      !  if(cs.lt.0) print*,'here cs<0, for sec'
      gs = BallBerry(Anet, rh, cs, pspar)
      gp = gs*mol_to_ml
      Ra = 1.d0/(gb*mol_to_ma)
      !!call  o3dep_uptake(Ra, gp, pspar%pft, o3, .true., F, Fo3, dFo3)    !!!! high sensitive
      call  o3dep_uptake(Ra, gp, pspar%pft, o3, .false., F, Fo3, dFo3)   !!! low sensitive
...
!!    else
      ...
      cs = ca - Anet*1.37d0/gb  !!! xiaoxie
       ! if(cs.lt.0) print*,'here cs<0, for third',ca,Anet,gb
      gs = BallBerry(Anet, rh, cs, pspar)
      ci = cs - Anet/(gs/1.65d0)

      Atot0 = Atot
#ifdef O3DEP_UPTAKE_OFFLINE
c      print*,'here doing o3 damage',Atot,F
      Atot0 = Atot*F
#endif
...
!     &     /(Rgas*(Tl+Kelvin))))
      ! print*,"Vcmax,tl,facclim",pftpar(p)%Vcmax, Tl, facclim
