...
!!! xiaoxie 2017 added
  subroutine read_yibs_driver(vlai,vfrac,vlandf,vheight,vlaix,frac_isl,&
    ...
    real(rkx) , pointer , dimension(:,:) , intent(inout) :: mvegid
    ...
    if ( do_parallel_netcdf_in ) then
      ...
      call read_var2d_static(idmin,'mvegid',rspace,istart=istart,icount=icount)
      mvegid(jci1:jci2,ici1:ici2) = rspace
      ...
      if ( myid == iocpu ) then
        ...
        call read_var2d_static(idmin,'mvegid',rspace,istart=istart,icount=icount)
        call grid_distribute(rspace,mvegid,jci1,jci2,ici1,ici2)
        call read_var2d_static(idmin,'cropdata',rspace,istart=istart,icount=icount)
